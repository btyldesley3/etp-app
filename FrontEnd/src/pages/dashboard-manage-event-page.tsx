...
const DashboardManageEventPage: React.FC = () => {
    ...
    if (isEditMode && !isLoading && user?.access_token) {
      const fetchEvent = async () => {
        ...
        setEventData({
          ...
          startTime: event.startDateAndTime
            ? formatTimeFromDate(new Date(event.startDateAndTime))
            : undefined,
          endDate: event.endDateAndTime,
          endTime: event.endDateAndTime
            ? formatTimeFromDate(new Date(event.endDateAndTime))
            : undefined,
          venueDetails: event.venue,
          saleStartDate: event.saleStartDateAndTime,
          saleStartTime: event.saleStartDateAndTime
            ? formatTimeFromDate(new Date(event.saleStartDateAndTime))
            : undefined,
          saleEndDate: event.saleEndDateAndTime,
          saleEndTime: event.saleEndDateAndTime
            ? formatTimeFromDate(new Date(event.saleEndDateAndTime))
            : undefined,
          status: event.status,
          ticketTypes: event.ticketTypes.map((ticket) => ({
            ...
            price: ticket.price,
            totalTicketsAvailable: ticket.totalTicketsAvailable,
          })),
          createdAt: event.createdAt,
          updatedAt: event.updatedAt,
        });
        setEventDateEnabled(!!(event.startDateAndTime || event.endDateAndTime));
        setEventSalesDateEnabled(!!(event.saleStartDateAndTime || event.saleEndDateAndTime));
      };
      fetchEvent();
    }
  }, [id, user]);
  ...
  const handleEventUpdateSubmit = async (accessToken: string, id: string) => {
    const ticketTypes: UpdateTicketTypeRequest[] = eventData.ticketTypes.map(
      (ticketType) => {
        return {
          ...
          totalTicketsAvailable: ticketType.totalTicketsAvailable,
        };
      },
    );
    ...
    const request: UpdateEventRequest = {
      ...
      saleStartDateAndTime:
        eventData.saleStartDate && eventData.saleStartTime
          ? combineDateTime(eventData.saleStartDate, eventData.saleStartTime)
          : undefined,
      saleEndDateAndTime:
        eventData.saleEndDate && eventData.saleEndTime
          ? combineDateTime(eventData.saleEndDate, eventData.saleEndTime)
          : undefined,
      ...
      <div className="container mx-auto px-4 py-8 max-w-xl">
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div>
            ...
            <DateTimeSelect
              date={eventData.saleStartDate}
              setDate={(date) => updateField("saleStartDate", date)}
              time={eventData.saleStartTime}
              setTime={(time) => updateField("saleStartTime", time)}
          ...
          <div>
            ...
            <DateTimeSelect
              date={eventData.saleEndDate}
              setDate={(date) => updateField("saleEndDate", date)}
              time={eventData.saleEndTime}
              setTime={(time) => updateField("saleEndTime", time)}
              enabled={eventSalesDateEnabled}
              setEnabled={setEventSalesDateEnabled}
            />
            <p className="text-gray-400 text-xs">
                      ...
                      <div className="bg-gray-700 w-full p-4 rounded-lg border-gray-600">
                        <div className="flex justify-between items-center">
                          ...
                          <div>
                            ...
                            {ticketType.totalTicketsAvailable && (
                              <p className="text-gray-400">
                                {ticketType.totalTicketsAvailable} tickets available
                              </p>
                            )}
                          </div>
                          {/* Right */}
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="space-y-1 w-full">
                      ...
                      <Input
                        ...
                        type="number"
                        value={currentTicketType?.totalTicketsAvailable}
                        onChange={(e) =>
                          setCurrentTicketType(
                            currentTicketType
                              ? {
                                  ...currentTicketType,
                                  totalTicketsAvailable: Number.parseFloat(
                                    e.target.value,
                                  ),
                                }
                              : undefined,
